...
import {
  ...
  clearPulseSelection,
  selectPulseHost,
} from '@/features/endge-admin/model/pulse/pulse.mock.ts'
import DomainEntityDropTarget from '@/features/endge-admin/ui/components/DomainEntityDropTarget.vue'
import type { PulseMockHost } from '@/features/endge-admin/model/pulse/pulse.mock.ts'
...
function rootIcon(hostType: string): string {
  ...
  const icon = EndgeAdmin.tabs.getDocumentIcon(hostType as any)
  return icon
...
function openDomainEntity(hostId: string): void {
  ...
  if (entityType === 'project' || entityType === 'view' || entityType === 'page' || entityType === 'action') {
    EndgeAdmin.tabs.openDocument(identity, entityType as any)
    return
  }

  if (entityType === 'table') {
    EndgeAdmin.tabs.openDocument(identity, ComponentType.Table as any)
    return
  }
  ...
  if (entityType === 'query') {
    ...
    EndgeAdmin.tabs.openDocument(identity, queryType as any)
    return
  }
  ...
  if (entityType === 'component') {
    ...
    EndgeAdmin.tabs.openDocument(identity, componentType as any)
    return
  }
