...
import { isDeleted as isEntityDeleted } from '@/features/endge-admin/model/domain/domain-tree'
...
function openStyle(styleId: string): void {
  EndgeAdmin.tabs.openDocument(styleId, 'style')
}

function openEntity(item: LeakItem): void {
  EndgeAdmin.tabs.openDocument(item.id, item.docType)
}
...
async function deleteLeak(item: LeakItem): Promise<void> {
  ...
  try {
    ...
    EndgeAdmin.tabs.closeTab(`${type}-${id}`)
