...
import type { DragPayloadItem } from '@/features/endge-admin/model/domain/domain-drag-drop'
import type { DomainDragTreeItem } from '@/features/endge-admin/model/domain/domain-drag-state'
import type { FlatFsItem, FsFileNode, FsFolderNode, FsNode } from '@/features/endge-admin/model/domain/domain-tree'
...
import { EndgeAdmin } from '@/features/endge-admin/model/core/endge-admin'
import {
  canHardDelete,
  canRestore,
  canSoftDelete,
  ...
  restoreFolder,
  softDeleteFolder,
} from '@/features/endge-admin/model/domain/domain-drag-drop'
import { clearDomainDrag, setDomainDrag } from '@/features/endge-admin/model/domain/domain-drag-state'
import {
  buildDomainTree,
...
async function onDrop(e: DragEvent, item: FlatFsItem): Promise<void> {
  ...
  const result = await EndgeAdmin.runBusy(executeDrop(payload, dropTarget))
...
function onRowClick(e: MouseEvent, item: FlatFsItem): void {
  ...
  if (node.sectionType === DomainSectionType.Settings) {
    EndgeAdmin.tabs.openSettingsProfile(String(targetId))
    return
  }
  if (node.sectionType === DomainSectionType.Project) {
    EndgeAdmin.tabs.openDocument(String(targetId), 'project')
    return
  }
  EndgeAdmin.tabs.openDocument(targetId, node.docType)
}
...
async function runMenuAction(a: MenuAction, ctxPath: string | null): Promise<void> {
  if (a.type === 'remove-folder') {
    await EndgeAdmin.runBusy(removeFolder(a.node))
    return
  }

  if (a.type === 'restore-folder') {
    await EndgeAdmin.runBusy(restoreFolderFromTrash(a.node))
    return
  }
  ...
  if (a.type === 'create-folder') {
    if (!ctxPath)
      return
    await EndgeAdmin.runBusy(createSubfolderUi(a.node, ctxPath))
    return
  }
  ...
  if (a.type === 'remove-doc') {
    await EndgeAdmin.runBusy(removeDocument(a.node, a.permanent))
    return
  }

  if (a.type === 'restore-doc') {
    await EndgeAdmin.runBusy(restoreDocument(a.node))
    return
  }

  if (a.type === 'duplicate-doc') {
    closeContextMenu()
    EndgeAdmin.modals.openDuplicateDocument({
      id: a.node.id,
  ...
  if (a.type === 'clear-soft-deleted') {
    EndgeAdmin.modals.openClearSoftDeleted()
  }
}
      ...
      <div class="px-2 py-1 flex items-center justify-between gap-1">
        <TooltipProvider :delay-duration="150">
          <div class="flex items-center gap-0.5">
            ...
            <Tooltip>
              <TooltipTrigger as-child>
                <Button size="icon" variant="ghost" class="size-7" :disabled="EndgeAdmin.busy.value" @click="save">
                  <Loader2 v-if="EndgeAdmin.busy.value" class="size-3.5 animate-spin" />
          ...
          <div class="flex items-center gap-0.5">
            <Tooltip>
              <TooltipTrigger as-child>
                <Button size="icon" class="size-7" @click="EndgeAdmin.modals.openCreateDocument()">
                  <Plus class="size-3.5" />
                </Button>
              </TooltipTrigger>
          ...
          >
            ...
            >
              ...
              <template v-else>
                ...
                <i
                  v-else
                  :class="EndgeAdmin.tabs.getDocumentIcon((it.node as FsFileNode).docType)"
    ...
    <Dialog v-model:open="renameDialog.open">
      <DialogContent class="sm:max-w-md">
        ...
        <DialogFooter class="gap-2">
          ...
          <Button @click="() => EndgeAdmin.runBusy(confirmRename())">
            Сохранить
          </Button>
        </DialogFooter>
      </DialogContent>
