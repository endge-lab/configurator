...
import { getSoftDeletedItems as getSoftDeletedFromTree } from '@/features/endge-admin/model/domain/domain-tree'
...
async function clearAll(): Promise<void> {
  ...
  loading.value = true
  try {
    await EndgeAdmin.runBusy((async () => {
    ...
    const closeTab = (id: string, docType: DomainDocumentType) => {
      EndgeAdmin.tabs.closeTab(`${docType}-${id}`)
    }
