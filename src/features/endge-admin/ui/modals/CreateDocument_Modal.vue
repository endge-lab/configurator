...
watch(() => props.open, (v) => {
  if (v) {
    const ctx = EndgeAdmin.modals.createDocumentContext?.value ?? null
...
async function onSubmit(): Promise<void> {
  ...
  try {
    if (createMode.value === 'json') {
      ...
      if (createdIdentity)
        EndgeAdmin.tabs.openDocument(createdIdentity, activeType.value)
    ...
    EndgeAdmin.tabs.openDocument(id, activeType.value)
        ...
        <div class="flex flex-col gap-2">
          ...
          <ScrollArea class="h-[240px] rounded-md border p-1">
            <div class="flex flex-col gap-0.5">
              ...
              >
                <i :class="EndgeAdmin.tabs.getDocumentIcon(doc.type)" class="text-lg shrink-0" />
