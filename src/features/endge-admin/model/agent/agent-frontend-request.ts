...
import { runAgentTableAction } from "@/features/endge-admin/model/agent/agent-table-actions";
import { setPulseActiveTab } from "@/features/endge-admin/model/pulse/pulse.mock.ts";
import { launchPulseRuntimeFromEntity } from "@/features/endge-admin/model/pulse/pulse.mock.ts";
import {
  duplicateEntity,
  getEntityByDocType,
} from "@/features/endge-admin/model/domain/domain-duplicate";
...
function resolveRuntimeIdentityFromActiveTab(
  ...
  const activeTab = EndgeAdmin.tabs.activeTab.value as {
    payload?: {
...
export function executeOpenDocumentChoice(
  ...
  try {
    if (documentType === "settings") {
      EndgeAdmin.tabs.openSettingsProfile(identity);
    } else {
      EndgeAdmin.tabs.openDocument(
        identity,
...
const SINGLETON_VIEW_METHODS: Record<string, () => void> = {
  ...
  "backup-restore": () => EndgeAdmin.tabs.openBackupRestoreSingleton(),
  demonstration: () => EndgeAdmin.tabs.openDemonstrationTab(),
  pulse: () => EndgeAdmin.tabs.openPulseTab(),
  diagnostics: () => {
    setPulseActiveTab("diagnostics");
    EndgeAdmin.tabs.openPulseTab();
  },
  "domain-analysis": () => EndgeAdmin.tabs.openDomainAnalysis(),
  architecture: () => EndgeAdmin.tabs.openArchitecture(),
};

function getActiveTableColumnTitles(): string[] {
  const ed = EndgeAdmin.tabs.documentEditorModel.value as {
