...
- Используется **RevoGrid** (`@revolist/vue3-datagrid`): библиотека уже предоставляет виртуализацию по строкам. Таблицы, построенные на RevoGrid (ComponentType_Table, ComponentType_TableV2, useRevoGrid, use-revogrid в @endge-vue), получают виртуализацию «из коробки».
...
- Все таблицы с потенциально большим числом строк (данные из запросов, журналы, справочники) должны использовать RevoGrid или другой виртуализированный грид. Не рендерить таблицы через обычный `<table>` с v-for по тысячам строк.
