...
The DOM backend lives in `@endge/vue`; Native DOM and Shadcn share it. It materializes declarations into stable generated classes and one managed stylesheet. Every generated class has the same non-zero browser specificity: Endge cascade is still resolved by declaration order, while author styles can override renderer/vendor tag defaults. Vue traversal supplies logical parent, sibling index, component boundary, evaluated props, state and part markers.
