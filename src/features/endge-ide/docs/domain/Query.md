...
props: defineProps({
  ...
  method: field('String').default('POST'),
  tenant: field('String').optional(),
  auth: field('Object').optional(),
  timeoutMs: field('Number').optional(),
  formUrlencoded: field('Boolean').default(false),
  filterPayload: field('Object').optional(),
  limit: field('Number').default(100),
})
```

Prop участвует в HTTP-запросе только при явной ссылке из `request`. `prop(...)` разрешён во всех runtime-полях request: `endpoint`, `path`, `method`, `headers`, `auth`, `timeoutMs`, `formUrlencoded` и `body`:
...
Static values and prop expressions can be mixed in one request. Composition owns the environment-specific choice and may pass it as a regular Query prop:

```ts
query('schedule').withProps({
  endpoint: env('ENDPOINT_AODB_SANDBOX'),
