...
<template>
  <Dialog :open="Boolean(prompt)" @update:open="value => { if (!value) preview.cancelAuthorization() }">
    <DialogContent class="sm:max-w-md">
      ...
      <p v-if="prompt?.error" class="text-sm text-destructive" role="alert">{{ prompt.error }}</p>
      <DialogFooter>
        <Button variant="outline" :disabled="prompt?.pending" @click="preview.cancelAuthorization()">Отмена</Button>
