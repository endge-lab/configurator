...
const props = defineProps<{
  tabContext?: {
    document?: {
      editor?: { identity: string; displayName: string; code: string; description: string }
      previewModel?: any
      component?: any
    }
  }
    ...
    <ScrollArea class="flex-1">
      <div class="p-4 space-y-4">
        ...
        <div class="space-y-2">
          ...
          <Input
            :model-value="editor?.code ?? model?.code ?? ''"
            :disabled="isSystem"
            placeholder="например: TENANT_DEFAULT"
            @update:model-value="(v) => editor && (editor.code = String(v ?? ''))"
          />
        </div>
        <div class="space-y-2">
