...
async function loadVocab(): Promise<void> {
  ...
  try {
    ...
    if (error instanceof AuthInteractionRequiredError) {
      toast.error('Для загрузки словаря требуется авторизация', {
        description: error.message,
        action: {
          label: 'Авторизоваться и повторить',
          onClick: () => void authorizeAndReload(error.profileIdentity),
        },
      })
    }
    else {
...
async function authorizeAndReload(profileIdentity: string): Promise<void> {
  try {
    await authorizeOidcProfile(profileIdentity)
    await loadVocab()
  }
  catch (error) {
    toast.error('Не удалось завершить вход для словаря', {
      description: error instanceof Error ? error.message : String(error),
    })
  }
}
