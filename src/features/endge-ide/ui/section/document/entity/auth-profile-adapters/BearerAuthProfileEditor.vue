...
const token = computed({ get: () => String(props.credentials.token ?? ''), set: value => emit('update:credentials', { token: value }) })
</script>

<template>
  <AuthCredentialField v-model="token" label="Bearer token" />
</template>
