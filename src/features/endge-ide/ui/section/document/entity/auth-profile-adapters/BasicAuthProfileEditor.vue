...
function credential(key: string) {
  return computed({ get: () => String(props.credentials[key] ?? ''), set: value => emit('update:credentials', { ...props.credentials, [key]: value }) })
}
...
<template>
  <div class="grid gap-3 sm:grid-cols-2">
    <AuthCredentialField v-model="username" label="Username" :secret="false" /><AuthCredentialField v-model="password" label="Password" />
  </div>
</template>
