...
export interface TypeVisualTypeOption {
  identity: string
  label: string
...
const definition = computed(() => parsed.value.document?.definition ?? null)
...
const errorDiagnostics = computed(() => parsed.value.diagnostics.filter(item => item.severity === 'error'))
const warningDiagnostics = computed(() => parsed.value.diagnostics.filter(item => item.severity === 'warning'))
...
function emitDocument(document: TypeSourceDocument): void {
  emit('update:modelValue', serializeTypeSourceDocument(document))
}

function mutateDocument(mutate: (document: TypeSourceDocument) => void): void {
  if (!parsed.value.document) { return }
  const next = cloneTypeSourceDocument(parsed.value.document)
  mutate(next)
  emitDocument(next)
}
...
function addObjectField(parentPath: number[] = [], afterIndex?: number): void {
  mutateDocument((document) => {
    ...
    fields.splice(index, 0, createDefaultTypeSourceField(key))
  })
}
...
function updateObjectFieldType(fieldPath: number[], value: string): void {
  mutateDocument((document) => {
    ...
    field.type = value === INLINE_OBJECT_VALUE
      ? { kind: 'object', fields: [createDefaultTypeSourceField('field')] }
...
function typeTone(identity: string): string {
  if (identity === 'String' || identity === 'ID' || identity === 'DateTime' || identity === 'Time') { return 'type-visual-editor__type--string' }
  if (identity === 'Number') { return 'type-visual-editor__type--number' }
  if (identity === 'Boolean') { return 'type-visual-editor__type--boolean' }
  if (identity === 'Null' || identity === 'Any') { return 'type-visual-editor__type--neutral' }
...
function sampleForExpression(expression: TypeSourceExpression, visited: Set<string>): unknown {
  ...
  if (identity === 'String' || identity === 'ID') { return 'string' }
  if (identity === 'Number') { return 0 }
  if (identity === 'Boolean') { return false }
...
<template>
  <div class="type-visual-editor">
    <div v-if="!parsed.valid || !definition" class="type-visual-editor__invalid">
      ...
      <div>
        ...
        <p class="mt-1 max-w-xl text-xs leading-5 text-muted-foreground">
          Исправьте Type Source, чтобы visual editor снова мог построить semantic document.
        </p>
            ...
            <div class="type-visual-editor__schema-card">
              ...
              <div v-if="definition.kind === 'object'" class="type-visual-editor__tree">
                <div
                  ...
                  :draggable="row.local && row.fieldPath != null"
                ...
                >
                  <div class="type-visual-editor__tree-main" :style="{ paddingLeft: `${18 + row.depth * 22}px` }">
                    <GripVertical v-if="row.local" class="type-visual-editor__drag size-3.5" />
                    ...
                    <div
                      v-if="row.local && row.fieldPath && canSelectFieldType(row.field)"
                      class="contents"
                    ...
                    <div class="type-visual-editor__row-actions">
                      <template v-if="row.local">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" class="w-52">
                            ...
                            <DropdownMenuItem @select="openDescriptionDialog(row)">
                              Добавить описание
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      ...
                      <button v-else type="button" aria-label="Open referenced type" @click.stop="emit('open:type', row.ownerIdentity)">
                ...
                <button v-if="!objectRows.length" type="button" class="type-visual-editor__empty-row" @click="addObjectField()">
                  <Plus class="size-4" />
