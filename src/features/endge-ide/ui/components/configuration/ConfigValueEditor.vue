...
const catalog = computed(
  () => props.catalog ?? Endge.configurationSchema.typeCatalog,
)
const registeredType = computed(() => {
  const type = props.type
  return type.kind === 'reference'
    ? catalog.value.find(item => item.identity === type.identity)
    : undefined
})
const effectiveType = computed<TypeSourceExpression>(
  () => registeredType.value?.definition ?? props.type,
)
const kind = computed(() => resolveConfigValueEditor(effectiveType.value))
const enumValues = computed(() =>
  effectiveType.value.kind === 'enum' ? effectiveType.value.values : [],
)
const objectFields = computed(() =>
  effectiveType.value.kind === 'object' ? effectiveType.value.fields : [],
)
const arrayItemType = computed(() =>
  effectiveType.value.kind === 'array'
    ? effectiveType.value.items
    : { kind: 'reference' as const, identity: 'Any' },
)
const unionVariants = computed(() =>
  effectiveType.value.kind === 'union' ? effectiveType.value.variants : [],
)
const recordValueType = computed(() =>
  effectiveType.value.kind === 'record'
    ? effectiveType.value.values
    : { kind: 'reference' as const, identity: 'Any' },
)
const unionIndex = computed(() =>
  Math.max(
    0,
    unionVariants.value.findIndex(
      variant =>
        validateConfigurationValue(variant, props.modelValue, catalog.value).ok,
    ),
  ),
)
const referenceType = computed(() =>
  registeredType.value?.category === 'reference' ? registeredType.value : null,
)
function emitText(value: unknown): void {
  emit('update:modelValue', String(value ?? ''))
}
function emitNumber(value: unknown): void {
  emit('update:modelValue', Number(value))
}
function triggerValue(): ComponentSFCInteractionTrigger[] {
...
function objectValue(): Record<string, EndgeJSONValue> {
...
function arrayValue(): EndgeJSONValue[] {
  return Array.isArray(props.modelValue) ? props.modelValue : []
}
function updateObjectField(key: string, value: EndgeJSONValue): void {
  emit('update:modelValue', { ...objectValue(), [key]: value })
}
function updateArrayItem(index: number, value: EndgeJSONValue): void {
  const next = [...arrayValue()]
  next[index] = value
  emit('update:modelValue', next)
}
function addArrayItem(): void {
  const inferred = inferConfigurationDefault(
    arrayItemType.value,
    catalog.value,
  )
  emit('update:modelValue', [
    ...arrayValue(),
    inferred.ok ? inferred.value : null,
  ])
}
...
function addRecordEntry(): void {
  ...
  emit('update:modelValue', {
    ...current,
    [`key${index}`]: inferred.ok ? inferred.value : null,
  })
}
function renameRecordEntry(previous: string, nextRaw: unknown): void {
  const next = String(nextRaw ?? '').trim()
  if (!next || next === previous || next in objectValue()) {
    return
  }
  ...
  emit(
    'update:modelValue',
    Object.fromEntries(entries) as Record<string, EndgeJSONValue>,
  )
}
function renameRecordEntryFromEvent(previous: string, event: Event): void {
  renameRecordEntry(previous, (event.target as HTMLInputElement | null)?.value)
}
function removeRecordEntry(key: string): void {
  emit(
    'update:modelValue',
    Object.fromEntries(
      Object.entries(objectValue()).filter(([item]) => item !== key),
    ) as Record<string, EndgeJSONValue>,
  )
}
...
function fieldType(
  field: (typeof objectFields.value)[number],
): TypeSourceExpression {
...
<template>
  ...
  <Select
    ...
    :disabled="disabled"
    @update:model-value="$emit('update:modelValue', JSON.parse(String($event)))"
  >
  ...
  <ConfigurationReferenceValueEditor
    v-else-if="referenceType"
  ...
  <div v-else-if="kind === 'union'" class="space-y-2">
    <Select
      :model-value="String(unionIndex)"
      :disabled="disabled"
      @update:model-value="selectUnion(String($event))"
    >
      <SelectTrigger><SelectValue /></SelectTrigger><SelectContent>
        <SelectItem
          v-for="(variant, index) in unionVariants"
          :key="index"
          :value="String(index)"
        >
          Вариант {{ index + 1 }} ·
    ...
    <ConfigValueEditor
      v-if="unionVariants[unionIndex]"
      :model-value="modelValue"
      :type="unionVariants[unionIndex]!"
      :catalog="catalog"
      :disabled="disabled"
      @update:model-value="$emit('update:modelValue', $event)"
    />
  ...
  <div v-else-if="kind === 'record'" class="space-y-2">
    <div
      v-for="(item, key) in objectValue()"
      :key="key"
      class="grid grid-cols-[10rem_minmax(0,1fr)_auto] gap-2 rounded-md border p-2"
    >
      <Input
        :model-value="key"
        :disabled="disabled"
        @change="renameRecordEntryFromEvent(key, $event)"
      />
      <ConfigValueEditor
        :model-value="item"
