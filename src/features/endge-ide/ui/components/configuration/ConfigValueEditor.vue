...
function emitTriggers(value: ComponentSFCInteractionTrigger[]): void {
  emit('update:modelValue', JSON.parse(JSON.stringify(value)) as EndgeJSONValue)
}
function objectValue(): Record<string, EndgeJSONValue> {
  return props.modelValue && typeof props.modelValue === 'object' && !Array.isArray(props.modelValue) ? props.modelValue : {}
}
...
function removeArrayItem(index: number): void {
  emit('update:modelValue', arrayValue().filter((_, itemIndex) => itemIndex !== index))
}
...
function removeRecordEntry(key: string): void {
  emit('update:modelValue', Object.fromEntries(Object.entries(objectValue()).filter(([item]) => item !== key)) as Record<string, EndgeJSONValue>)
}
...
<template>
  ...
  <Select v-else-if="kind === 'enum'" :model-value="JSON.stringify(modelValue)" :disabled="disabled" @update:model-value="$emit('update:modelValue', JSON.parse(String($event)))">
    ...
    <SelectContent>
      <SelectItem v-for="item in enumValues" :key="JSON.stringify(item)" :value="JSON.stringify(item)">
        {{ String(item) }}
      </SelectItem>
    </SelectContent>
  </Select>
  ...
  <div v-else-if="kind === 'array'" class="space-y-2">
    <div v-for="(item, index) in arrayValue()" :key="index" class="grid grid-cols-[minmax(0,1fr)_auto] gap-2 rounded-md border p-2">
      ...
      <Button type="button" size="icon" variant="ghost" :disabled="disabled" @click="removeArrayItem(index)">
        <Trash2 class="size-4" />
      </Button>
    </div>
    <Button type="button" size="sm" variant="outline" :disabled="disabled" @click="addArrayItem">
  ...
  <div v-else-if="kind === 'union'" class="space-y-2">
    <Select :model-value="String(unionIndex)" :disabled="disabled" @update:model-value="selectUnion(String($event))">
      <SelectTrigger><SelectValue /></SelectTrigger><SelectContent>
        <SelectItem v-for="(variant, index) in unionVariants" :key="index" :value="String(index)">
          Вариант {{ index + 1 }} · {{ variant.kind === 'reference' ? variant.identity : variant.kind }}
        </SelectItem>
      </SelectContent>
    </Select>
  ...
  <div v-else-if="kind === 'record'" class="space-y-2">
    <div v-for="(item, key) in objectValue()" :key="key" class="grid grid-cols-[10rem_minmax(0,1fr)_auto] gap-2 rounded-md border p-2">
      ...
      <Button type="button" size="icon" variant="ghost" :disabled="disabled" @click="removeRecordEntry(key)">
        <Trash2 class="size-4" />
      </Button>
    </div>
    <Button type="button" size="sm" variant="outline" :disabled="disabled" @click="addRecordEntry">
