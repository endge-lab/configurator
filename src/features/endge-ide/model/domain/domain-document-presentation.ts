...
import { QUERY_COMPOSITION_PRESENTATION_KIND } from './query-composition-presentation'
...
export const DOMAIN_SECTION_PRESENTATION: Record<DomainSectionType, DomainDocumentPresentation> = {
  ...
  [DomainSectionType.View]: { icon: 'Eye', colorClass: 'text-indigo-500' },
...
export function getDomainDocumentPresentation(
  ...
  if (String(docType) === 'composition' && presentationKind === QUERY_COMPOSITION_PRESENTATION_KIND) {
    return {
      ...presentation,
      colorClass: getDomainSectionPresentation(DomainSectionType.Query).colorClass,
    }
  }
