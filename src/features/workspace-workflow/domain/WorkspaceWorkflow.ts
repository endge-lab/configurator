...
export class WorkspaceWorkflow {
  ...
  public replaceRoots(roots: WorkflowDependency[]): void {
    ...
    this._expandedResources = new Set([...this._graph.resources.keys()].filter(id =>
  ...
  public resetLayout(): void {
    if (this._layout) {
      ...
      this._expandedResources = new Set(this._graph.resources.keys())
    }
  }
  ...
  private _buildScene(): { nodes: WorkflowNode[], edges: WorkflowEdge[] } {
    ...
    const measure = (id: string, depth: number): void => {
      ...
      const branches = children.filter(child => !stores.includes(child)
        && (graph.nodes.get(child)?.kind === 'project' || graph.nodes.get(child)?.kind === 'composition' || graph.nodes.get(child)?.kind === 'scope' || graph.children.get(child)?.length))
