...
describe('форматирование Source', () => {
  it('форматирует полный Vue SFC, включая блоки TypeScript и template', async () => {
...
`)
  ...
  it('форматирует самостоятельный EndgeCSS с его SCSS-совместимой грамматикой', async () => {
...
`)
  ...
  it('сохраняет селекторы Endge DataPath вместо преобразования в присваивания', async () => {
  ...
  it('исправляет селекторы DataPath, обёрнутые предыдущим проходом Prettier', async () => {
  ...
  it('нормализует директивы управления потоком Vue в канонический синтаксис Endge SFC', async () => {
  ...
  it('форматирует правила, директивы, вложенность и комментарии EndgeCSS внутри SFC', async () => {
...
Table{&::part(cell){border-right:1px solid #355a82}}</style>`
  ...
  it('форматирует блок style без lang как EndgeCSS и не добавляет lang', async () => {
    const formatted = await formatSource(
