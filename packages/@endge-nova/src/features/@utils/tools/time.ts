...
export function formatDatetime(date: Date, formatString = 'HH:mm dd.MM.yyyy', options = {}): string {
...
export function formatDateToMSK(date): string {
...
export function extractTime(date: Date | string | object, isLocalTime = true): string {
  return formatDatetimeTZ(date, 'HH:mm', {}, isLocalTime)
}

export function findMinMaxDates(dates: Set<Date>): {
  minDate: Date
  maxDate: Date
} {
